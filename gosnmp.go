...
type GoSNMP struct {
	...
	// Transport is the transport protocol to use ("udp" or "tcp" or "quic"); if unset "udp" will be used.
	Transport string
	...
	// for Transport of QUIC(from quic_transport.go)
	QuicTransport *QuicTransport

	// If self-certification is used, InsecureMode should be set to “true”.
	InsecureMode bool
}
...
func (x *GoSNMP) connect(networkSuffix string) error {
	...
	switch x.Transport {
	case "quic":
		if err = x.netConnectQUIC(); err != nil {
			return fmt.Errorf("error esatablishing connection QUIC: %w", err)
		}
	default:
...
func (x *GoSNMP) netConnectQUIC() error {
	// if x.QuicTransport == nil {
	// 	return fmt.Errorf("QUIC transport not initialized")
	// }
	var err error
	var t *QuicTransport
	addr := net.JoinHostPort(x.Target, strconv.Itoa(int(x.Port)))
	if t, err = ConnectQUIC(addr, x.InsecureMode); err != nil {
		return err
	}
	x.QuicTransport = t
	return nil
...
func (x *GoSNMP) SnmpEncodeGetResponsePacket(requestID uint32, errorIndex int32, pdus []SnmpPDU) ([]byte, error) {
	err := x.validateParameters()
	if err != nil {
		return []byte{}, err
	}

	packet := x.mkSnmpPacket(GetResponse, pdus, 0, 0)
	packet.RequestID = requestID
	if errorIndex >= 0 {
		packet.Error = NoSuchName
		packet.ErrorIndex = uint8(errorIndex)
	}

	if x.Version == Version3 {
		packet.MsgID = requestID
		err = x.initPacket(packet)
		if err != nil {
			return []byte{}, err
		}
	}
