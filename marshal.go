...
func (x *GoSNMP) sendOneRequest(packetOut *SnmpPacket,
	...
	for retries := 0; ; retries++ {
		...
		switch x.Transport {
		case "quic":
			err = x.QuicTransport.stream.SetDeadline(reqDeadline)
			if err != nil {
				return nil, err
			}
		default:
		...
		switch x.Transport {
		case "quic":
			_, err = x.QuicTransport.stream.Write(outBuf)
			if err != nil {
				continue
			}
		default:
...
func (x *GoSNMP) send(packetOut *SnmpPacket, wait bool) (result *SnmpPacket, err error) {
	...
	case "quic":
		if x.QuicTransport == nil {
			return nil, fmt.Errorf("&GoSNMP.QuicTransport is missing. Provide a connection or use Connect()")
		}
	default:
		if x.Conn == nil {
...
func (x *GoSNMP) receive() ([]byte, error) {
	...
	switch x.Transport {
	case "quic":
		// stream, err = x.QuicTransport.conn.AcceptStream(context.Background())
		// if err != nil {
		// return nil, err
		// }
		// n, err = stream.Read(x.rxBuf[:])
		n, err = x.QuicTransport.stream.Read(x.rxBuf[:])
		if err == io.EOF {
			return nil, err
		} else if err != nil {
